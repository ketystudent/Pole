"""Lote de teste (fase 1, v2): reprocessa os mesmos 20 postes do lote 01,
agora usando o rumo geometrico (bearing camera->poste cadastrado) para
restringir a busca do angulo a uma janela de +-55 graus, em vez de varrer
os 360 graus inteiros. Objetivo: comparar a taxa de acerto antes/depois.
"""
import csv
import json
import os
...
def main():
    items = load_batch()
    print(f"reprocessando {len(items)} postes do lote 01 com busca restrita por rumo geometrico (janela +-{WINDOW_DEG} graus)")
    log = []
    ...
    print(f"\nconcluido: {ok}/{len(log)} sem erro tecnico ({warn} sinalizados p/ conferir centralizacao)")
