...
def classify(twin_edge, wire_count):
    # v4: contagem de fios manda mais que o par de bordas (que se mostrou
    # rigido demais e descartou fotos boas no v3, ex: 350236666, 1358664738,
    # onde o poste era real mas nao formou duas bordas paralelas limpas).
    if wire_count >= 3:
        return "ok", ""
    if twin_edge and wire_count >= 2:
        return "ok", ""
    if twin_edge or wire_count >= 1:
        return "poste_casa", "poucos fios no topo - possivel ramal residencial, confirmar"
    return "baixa_confianca", "sem par de bordas nem fios confiaveis - possivel arvore, sinalizacao ou objeto nao identificado"
...
def main():
    ...
    out_csv = r"c:\Users\ksenne\OneDrive - azureford\Desktop\Project\Pole\lote_teste_04_log.csv"
